...
int count_days(int tomato[100][100][100], int visit[100][100][100], queue<pair<pair<int, int>, int> > que, int M, int N, int H)
{
	int max_day = 1;
	...
	visit[cur.first.first][cur.first.second][cur.second] = 1;
	while (!que.empty()){
	pair<pair<int, int>, int> cur = que.front();
	que.pop();
...
int main()
{
	...
	queue<pair<pair<int, int>, int> > start;
	int max_day;
	...
	for (int m = 0; m < M; m++){
		for (int n = 0; n < N; n++){
			for (int h = 0; h < H; h++){
				cin >> tomato[m][n][h];
				if (tomato[m][n][h] == 1){
					start.push(make_pair(make_pair(m, n), h));
					visit[m][n][h] = 1;
				}
			}
		}
	}
	max_day = count_days(tomato, visit, start, M, N, H);
	//print_visit(visit, M, N, H);
	//print_visit(tomato, M, N, H);
