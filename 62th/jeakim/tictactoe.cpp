...
int check_x(string in)
{
	for (int i = 0; i < 3; i++)
	{
		if ((in[i * 3] == 'X') && (in[i * 3 + 1] == 'X') && (in[i * 3 + 2] == 'X'))  //가로
			return (1);
		if ((in[i] == 'X') && (in[3 + i] == 'X') && (in[6 + i] == 'X')) //세로
			return (1);
	}
	if ((in[2] == 'X') && (in[4] == 'X') && (in[6] == 'X')) //대각선
		return (1);
	if ((in[0] == 'X') && (in[4] == 'X') && (in[8] == 'X')) //대각선
		return (1);
	return (-1);
}
...
int check_o(string in)
{
	...
	if ((in[0] == 'O') && (in[4] == 'O') && (in[8] == 'O')) //대각선
		return (0);
	if ((in[2] == 'O') && (in[4] == 'O') && (in[6] == 'O'))  //대각선
		return (0);
	return (-1);
}
